...
function walkValue(ast, visit, options = {}, parents = []) {
...
function formatCSS(css, options = {}) {
    ...
    css = css.replace(/\r?\n|\r/g, endOfLine);
    const ast = (0, postcss_1.parse)(css);
...
function formatAst(ast, index, options) {
    const { endOfLine: NL, indent, indentLevel, linesBetween, endWithNewline, wrapLineLength, } = options;
    if (ast.type === 'rule') {
        ...
        if (ast.variable) {
            ...
            const valueAst = (0, css_value_parser_1.parseCSSValue)(value);
            const preserveComponentNewLines = hasNewLineBeforeValue || isDeclComponentPreservedNewLines(ast);
            ...
            walkValue(valueAst, normalizeDeclValueAndCollectLength(preserveComponentNewLines, NL, nodesInfo), { insideOut: true });
        ...
        const separation = (childrenLen === -1 && !hasRuleBefore) || hasCommentBefore ? 0 : linesBetween;
        ast.raws.before =
            index !== 0 || indentLevel > 0
...
class AtRuleParamFormatter {
     ...
     */
    format(params) {
        const paramsAst = (0, css_value_parser_1.parseCSSValue)(params);
        ...
        return (0, css_value_parser_1.stringifyCSSValue)(paramsAst);
    }
    getContext(node, parents, siblings) {
    ...
    formatWhitespace(node) {
        node.before = node.after = '';
        if (node.value && !node.value.includes('\n')) {
            node.value = ' ';
        }
    }
    formatAroundComma(_node, context, options) {
    ...
    formatBlockOpen(node, { nextNode }, { endOfLine: NL }) {
        const isSpaceAfter = nextNode?.type === 'space';
        const isNewline = isSpaceAfter && (0, css_value_parser_1.stringifyCSSValue)(nextNode).includes('\n');
        if (isSpaceAfter) {
            if (isNewline) {
                this.increaseIndent();
            }
    ...
    formatBlockClose(node, context, options) {
        const { prevNode, currentBlock } = context;
        if (currentBlock.type === node.value) {
            this.blockCloserStack.pop();
            if (currentBlock.isNewline) {
                this.currentNewlineIndent = this.currentNewlineIndent.slice(0, this.currentNewlineIndent.length - options.indent.length);
            }
        }
        this.setNewlineOrInline({
            spaceNode: prevNode,
    ...
    formatFunction(node, { endOfLine: NL }) {
    ...
    setNewlineOrInline({ spaceNode, placeBefore, inlineSpace, }, { currentBlock, siblings, index }, { endOfLine: NL }) {
        ...
        if (spaceNode?.type === 'space') {
            ...
            siblings.splice(index + (placeBefore ? 0 : 1), 0, (0, css_value_parser_1.space)({ value: spaceValue }));
        }
    }
    addToBlockStack(node, isNewline) {
        if (node.type === 'call') {
            this.blockCloserStack.push({ type: 'func-end', isNewline, node });
        }
        else if (node.value in blockCloserMap) {
            this.blockCloserStack.push({
                type: blockCloserMap[node.value],
                isNewline,
            });
        }
    }
    getCurrentStackBlock(parents) {
        ...
        while (currentBlock?.type === 'func-end' && !parents.includes(currentBlock.node)) {
            this.blockCloserStack.pop();
            if (currentBlock.isNewline) {
                this.currentNewlineIndent = this.currentNewlineIndent.slice(0, this.currentNewlineIndent.length - this.options.indent.length);
            }
...
function normalizeDeclValueAndCollectLength(preserveTopLevelNewLines, NL, nodesInfo) {
    return (node, parents, siblings) => {
        ...
        if (node.type === 'space') {
            const isNewLine = preserveNewLines &&
                (node.value.includes('\n') ||
        ...
        if (node.type === 'literal' && node.value === ',') {
            ...
            if (nextNode?.type !== 'space') {
                siblings.splice(index + 1, 0, (0, css_value_parser_1.space)({ value: ' ' }));
            }
            // remove space before
            if (prevNode?.type === 'space') {
        ...
        nodesInfo.set(node, { length: (0, css_value_parser_1.stringifyCSSValue)(node).length, breakFuncArgs });
    };
}
function isDeclComponentPreservedNewLines({ prop }) {
    return prop === 'grid-template-areas' || prop === 'grid-template';
}
function flowDeclValueSegment({ nodes, maxLength, nodesInfo, singleIndent, baseIndentSize, NL, breakOnComma, }) {
    const baseIndent = ' '.repeat(baseIndentSize);
    ...
    for (let index = 0; index <= originalNodes.length - 1; ++index) {
        ...
        const breakableFunction = isFunction && isFunctionBreakable(node, nodesInfo);
        ...
        if (node.type === 'space' && node.value.includes('\n')) {
            ...
            if (node.type === 'space') {
                ...
                if (prevNode?.type === 'space') {
                    ...
                    nodes.splice(currentNodeIndex, 0, (0, css_value_parser_1.space)({ value: NL + baseIndent }));
                    currentNodeIndex++;
                }
            }
        }
...
function isFunctionBreakable(funcNode, nodesInfo) {
...
function splitAndIndentDeclFuncArgs({ funcNode, maxLength, nodesInfo, singleIndent, baseIndentSize, NL, }) {
...
function enforceOneSpaceAround(value) {
...
function cleanValue(value, forceSpaceInSpaceNode = false) {
    return (0, css_value_parser_1.stringifyCSSValue)(cleanValueAst((0, css_value_parser_1.parseCSSValue)(value), forceSpaceInSpaceNode));
}
function cleanValueAst(ast, forceSpaceInSpaceNode = false) {
    for (const node of ast) {
        if ('before' in node) {
...
function parseDeclBetweenRaws(between) {
    const beforeAst = (0, css_value_parser_1.parseCSSValue)(between);
    const beforeNode = {
        preSpace: '',
        preComments: [],
        postSpace: '',
    ...
    for (const node of beforeAst) {
        if (node.type === 'space') {
            if (beforeNode.colon) {
                beforeNode.postSpace += (0, css_value_parser_1.stringifyCSSValue)(node);
            }
            else {
                beforeNode.preSpace += (0, css_value_parser_1.stringifyCSSValue)(node);
            }
        }
...
function formatSelectors(rule, forceNL, { endOfLine: NL, indent, indentLevel }) {
...
function groupBySize(parts, joinWith = ', ') {
    const maxLength = 50;
    const grouped = [];
    let currentGroup = [];
